---
title: "The United States Senate"
subtitle: "The Senate gives every state an equal voice in Congress."
hero:
  src: "assets/visuals/heroes/11-senate.svg"
  alt: "The number 100 with paired seats, representing two senators per state"
diagram:
  src: "svg/civic-learning-map.svg"
  alt: "Visual diagram for The United States Senate"
objectives:
  - "Explain why each state has two Senators"
  - "Describe Senate terms"
  - "Understand unique Senate powers"
callout: "There are 100 Senators."
summary:
  - "There are 100 Senators."
  - "Senators serve six years."
  - "The Senate confirms appointments and approves treaties."
terms:
  - "Constitution"
  - "Government"
  - "Rights"
  - "Citizenship"
  - "Federalism"
  - "Representation"
  - "Rule of Law"
review:
  - "What is the main idea of this chapter?"
  - "How does this chapter connect to the Constitution?"
  - "Why is this topic important for citizenship?"
  - "Give one real-life example from this chapter."
---

## The States’ Voice

Every state has two Senators regardless of population. This protects smaller states and balances the House.

## Six-Year Terms

Senators serve six-year terms, and only about one-third are elected every two years. This creates stability and continuity.

## Unique Powers

The Senate confirms major presidential appointments, approves treaties, and conducts impeachment trials.

## Direct Election

Originally state legislatures chose Senators. The Seventeenth Amendment created direct election by voters.
