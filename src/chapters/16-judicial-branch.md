---
title: "The Judicial Branch"
subtitle: "The courts interpret laws and protect the Constitution."
diagram:
  src: "svg/civic-learning-map.svg"
  alt: "Visual diagram for The Judicial Branch"
figures:
  - src: "assets/visuals/timelines/landmark-supreme-court.svg"
    caption: "Landmark Supreme Court decisions"
    alt: "Timeline: 1803 Marbury v. Madison, 1896 Plessy v. Ferguson, 1954 Brown v. Board, 1966 Miranda v. Arizona, 1974 U.S. v. Nixon"
objectives:
  - "Describe the federal court system"
  - "Explain judicial review"
  - "Understand judicial independence"
callout: "The Supreme Court is the highest court."
visual_pack: "svg/milestone3/constitution-blueprint.svg"
summary:
  - "The Supreme Court is the highest court."
  - "Judicial review protects the Constitution."
  - "Judges are independent from the political branches."
terms:
  - "Constitution"
  - "Government"
  - "Rights"
  - "Citizenship"
  - "Federalism"
  - "Representation"
  - "Rule of Law"
review:
  - "What is the main idea of this chapter?"
  - "How does this chapter connect to the Constitution?"
  - "Why is this topic important for citizenship?"
  - "Give one real-life example from this chapter."
---

## Why Courts Exist

When there is a legal dispute or constitutional challenge, courts provide an independent place to resolve it.

## Court Structure

Federal cases may begin in District Courts, move to Courts of Appeals, and sometimes reach the Supreme Court.

## Judicial Review

Judicial review allows courts to determine whether laws or government actions comply with the Constitution. Marbury v. Madison established this principle.

## Independence

Federal judges are appointed by the President, confirmed by the Senate, and generally serve during good behavior so they can decide cases without election pressure.
